...
- If inside a class, stored on the heap as part of the class
- If local variable or method parameter, stored on the stack

**Usage:**
- Can be stored in arrays
- Can be fields in classes
- Can be boxed to object
- Can implement interfaces
- Can be used in async methods
- Can be used with LINQ

**Performance:**
- No garbage collection overhead when stack-allocated
- Can be boxed (converted to reference type) - causes heap allocation
- Copying creates a full copy of all fields
- Can cause defensive copies if passed without `in` keyword

**Best For:**
- Small, immutable data types (e.g., Point, Rectangle, Color)
- Types that represent a single value
- High-performance scenarios where GC pressure matters
- Types under 16 bytes in size
...
**Memory Allocation:**
- **ONLY stack-allocated** (never on heap)
- Cannot escape the stack
- Lifetime is tied to the stack frame
...
- ❌ Cannot be used with LINQ
- ❌ Cannot be captured by lambdas/closures
- ❌ Cannot be stored in arrays (heap-allocated)
- ✅ Can be used as local variables or method parameters
- ✅ Can contain regular struct fields

**Performance:**
- **Extremely fast** - no heap allocations ever
- No GC pressure
- Great for working with memory directly
- Perfect for buffer manipulation

**Best For:**
- Temporary data that doesn't outlive the method
- Working with `Span<T>` or `Memory<T>`
- High-performance buffer operations
- Parsing scenarios
- Stack-only computations
...
public readonly ref struct MyReadOnlyStruct
{
    ...
    // Cannot mutate - no Mutate() method possible
}
```

### Characteristics

**All restrictions of `ref struct` PLUS:**
- All fields must be `readonly`
- All methods are implicitly `readonly`
- Cannot modify state after construction
- Prevents defensive copies when passing by `in` parameter

**Performance Benefits:**
- Compiler guarantees immutability
- No defensive copies needed
- Most optimized form for passing by reference
- JIT can make additional optimizations

**Best For:**
- Immutable views over memory
- Zero-allocation string parsing
- High-performance read-only operations
- API contracts that guarantee no mutation
